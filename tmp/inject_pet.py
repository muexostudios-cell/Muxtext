...
(function _registerPetEggItems(){
  ...
  // Immediately patch item functions (ITEMS/ITEM_DISPLAY_ORDER may be scoped inside
  // the game's closure so we can't rely on them — we use direct renderInventoryPanel
  // monkey-patching instead for robust display).
  _patchItemFunctions();
  console.log('[PET SYSTEM] petEgg inventory patches applied');

  function _patchItemFunctions(){
    // ── Helper: build the hatch action overlay ─────────────────────────────
    function _showPetEggAction(typeId){
      const def = getPetTypeDef(typeId);
      const nm = (currentLang==='zh') ? def.nameZh : def.nameEn;
      const rc = RARITY_COLORS_PET[def.rarity] || '#aaa';
      const rarityLabel = {common:'普通',rare:'稀有',epic:'史詩',legendary:'傳說'}[def.rarity]||def.rarity;
      const petState = player && player.pet;
      ...
      const qty = getItemQty('petEgg_' + typeId);
      overlay.innerHTML = `
        <div style="background:#1a1a2e;border:1px solid ${rc};border-radius:8px;padding:1rem;min-width:200px;max-width:280px;text-align:center;">
          ...
          <div style="color:#888;font-size:0.55rem;margin-bottom:0.5rem;">${currentLang==='zh'?`稀有度：${rarityLabel}`:'Rarity: '+def.rarity} ${def.elem||''}</div>
          <div style="color:#666;font-size:0.5rem;margin-bottom:0.8rem;">x${qty}</div>
          ${hasPet
    ...
    // ── Patch renderInventoryPanel ─────────────────────────────────────────
    // We append petEgg items AFTER the original function runs, bypassing any
    // ITEM_DISPLAY_ORDER scope issues entirely.
    if(typeof renderInventoryPanel === 'function'){
      const _orig_renderInventoryPanel = renderInventoryPanel;
      renderInventoryPanel = function(){
        _orig_renderInventoryPanel.apply(this, arguments);
        _appendPetEggSlots();
      };
    }

    function _appendPetEggSlots(){
      // 'item-grid' is the confirmed element id used by the game's inventory panel
      const itemGrid = document.getElementById('item-grid');
      if(!itemGrid) return;

      // Remove any previously-appended petEgg slots so we don't duplicate
      itemGrid.querySelectorAll('.pet-egg-slot').forEach(el=>el.remove());

      const petEggAdded = [];
      PET_TYPES.forEach(function(def){
        const itemId = 'petEgg_' + def.id;
        const qty = getItemQty(itemId);
        if(qty <= 0) return;
        petEggAdded.push({def, itemId, qty});
      });
      if(petEggAdded.length === 0) return;

      petEggAdded.forEach(function({def, itemId, qty}){
        const rc = RARITY_COLORS_PET[def.rarity] || '#aaa';
        const nm = (currentLang==='zh') ? def.nameZh : def.nameEn;
        const div = document.createElement('div');
        div.className = 'equip-slot pet-egg-slot';
        div.style.cssText = `border-color:${rc}88;cursor:pointer;`;
        div.innerHTML = `<span class="item-entry-row"><span style="font-size:1.1rem;line-height:1;margin-right:4px;">🥚</span><span class="item-entry-text"><b style="color:${rc}">${nm}蛋</b> <span style="color:#666;">x${qty}</span></span></span>`;
        div.addEventListener('click', function(){ _showPetEggAction(def.id); });
        itemGrid.appendChild(div);
      });
    }

    // ── Patch applyDevAccountBootstrap ────────────────────────────────────
    // This is called right after initGame() loads the player save — the most
    // reliable moment to grant dev eggs.
    if(typeof applyDevAccountBootstrap === 'function'){
      const _orig_applyDevAccountBootstrap = applyDevAccountBootstrap;
      applyDevAccountBootstrap = function(){
        _orig_applyDevAccountBootstrap.apply(this, arguments);
        setTimeout(function(){
          const n = grantDevPetEggs();
          if(n > 0){
            console.log('[PET SYSTEM] Dev eggs granted via applyDevAccountBootstrap:', n);
            if(typeof renderInventoryPanel === 'function') renderInventoryPanel();
          }
        }, 50);
      };
